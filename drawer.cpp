...
#ifdef _VERBOSE_MODE
    sprintf(szCommandLine, "%s/bin/java -Dfile.encoding=UTF8 -jar %s/executor.jar -port %d -verbose true -kill %d", jarLoc, jarLoc, fxPort, GetCurrentProcessId());
#else
    sprintf(szCommandLine, "%s/bin/java -Dfile.encoding=UTF8 -jar %s/executor.jar -port %d -verbose false -kill %d", jarLoc, jarLoc, fxPort, GetCurrentProcessId());
#endif
